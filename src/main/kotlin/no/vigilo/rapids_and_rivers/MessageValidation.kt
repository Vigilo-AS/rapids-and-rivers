...
import no.vigilo.rapids_and_rivers.ValidationSpec.Companion.allAreOK
import no.vigilo.rapids_and_rivers.ValueValidation.Companion.optional
...
fun interface ValueValidation {
    ...
    companion object {
        fun ValueValidation.optional() = ValueValidation { node ->
            if (node.isMissingOrNull()) ValidationResult.Valid else validate(node)
        }
    }
}
...
class ValidationSpec(private val validation: ValueValidation, private val errorStrategy: MessageProblems.(String) -> Unit) {
    companion object {
        fun List<ValidationSpec>.allAreOK(key: String, valueToBeEvaluated: JsonNode, problems: MessageProblems): Boolean {
            return all { spec -> spec.validate(key, valueToBeEvaluated, problems) is ValidationResult.Valid }
        }
    }
    fun validate(key: String, node: JsonNode, problems: MessageProblems): ValidationResult {
        ...
        when (result) {
            ...
            is ValidationResult.Invalid -> errorStrategy(problems, "$key: ${result.message}")
        }
        return result
    }
}